...
struct sock {
    ...
    unsigned short  sk_num;       /* 本地端口 (网络字节序) */
    __u32           sk_daddr;     /* 外部 IPv4 地址 */
    __u32           sk_rcv_saddr; /* 本地 IPv4 地址 */
    unsigned short  sk_dport;     /* 外部端口 (网络字节序) */
};
...
static int hook_tcp_sendmsg(struct sock *sk, struct msghdr *msg, size_t size)
{
    if (monitor_running && sk) {
        char src[64], dst[64];
        snprintf(src, sizeof(src), "%pI4", &sk->sk_rcv_saddr);
        snprintf(dst, sizeof(dst), "%pI4", &sk->sk_daddr);
...
static long exit(void *__user reserved) { printk(KERN_INFO "KMS_NET: unloaded\n"); return 0; }
